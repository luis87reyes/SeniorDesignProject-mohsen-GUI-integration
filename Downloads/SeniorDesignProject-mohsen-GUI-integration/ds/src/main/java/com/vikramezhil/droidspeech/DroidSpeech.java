...
public class DroidSpeech
{
    ...
    private Button confirm, retry, retryFail;
    ...
    private String[] possibleWords = new String[0];
     ...
     * @param possibleWords The words that should be recognized
     */
    public DroidSpeech(Context context, FragmentManager fragmentManager, String [] possibleWords)
    {
    ...
    private void initRecognitionProgressView()
    {
        if(speechProgressAlertDialog == null)
        {
            try
            {
                ...
                retryFail = speechProgressView.findViewById(R.id.retryFail);
                retryFail.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                ...
                droidSpeechRecognizer.setRecognitionListener(new RecognitionListener() {
                    ...
                    public void onResults(Bundle results)
                    {
                        ...
                        if(valid)
                        {
                            ...
                            String preProcessedResult = results.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION).get(0);
                            String postProcessedResult = processResult(preProcessedResult);
                            String droidSpeechFinalResult;

                            if (postProcessedResult != null) {
                                droidSpeechFinalResult = postProcessedResult;
                                confirm.setVisibility(View.VISIBLE);
                                retry.setVisibility(View.VISIBLE);
                                retryFail.setVisibility(View.INVISIBLE);
                            }
                            else
                            {
                                confirm.setVisibility(View.INVISIBLE);
                                retry.setVisibility(View.INVISIBLE);
                                retryFail.setVisibility(View.VISIBLE);
                                droidSpeechFinalResult = "Sorry, I could not understand something you said";
                            }

                            setRecognitionProgressMsg(droidSpeechFinalResult);
    ...
    {
        ArrayList<String> resultArray = new ArrayList<String>(Arrays.asList(result.split(" ")));
        ArrayList<String> processedArray = new ArrayList<>();

        for (int i = 0; i < resultArray.size(); i++)
        {
            processedArray.add(SimilarityModel.findMostSimilarString(resultArray.get(i), possibleWords));
        }

        StringBuilder stringBuilder = new StringBuilder();

        //Can't use string.join here because it requires api lvl 26 :(
        for (int i = 0; i < processedArray.size(); i++)
        {
            stringBuilder.append(processedArray.get(i));
